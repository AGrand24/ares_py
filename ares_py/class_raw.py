...
from ares_py.electrodes import get_electrodes


class Raw:
    def __init__(self, fp):
        self.fp_load = fp
        self.line = int(Path(self.fp_load).stem)
        self.cs_res = None
    ...
    def Load(self):
        if self.fp_load.endswith(".2dm"):
            self = load_2dm(self)
            self.electrodes = get_electrodes(self)
        return self

    def Save(self, folder):
        "Saves data and electordes to ASCII in specified folder."
        self.data.to_csv(
            Path(folder, f"{str(self.line).zfill(3)}.apd"), index=False, sep="\t"
        )
        self.electrodes.to_csv(
            Path(folder, f"{str(self.line).zfill(3)}.ape"), index=False, sep="\t"
        )
        return self
